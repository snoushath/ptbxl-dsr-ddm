...
class DependencyGatedSequenceClassifier(nn.Module):
    def __init__(
        self,
        input_size: int = 12,
        d_model: int = 128,
    ...
    #     # Convert to: (batch, samples, leads)
    #     x = x.transpose(1, 2)
    ...
    #     return logits
    def forward(self, x: torch.Tensor,return_ddm_attention: bool = False,):
    # Input: (batch, leads, samples)
    # Convert to: (batch, samples, leads)
        x = x.transpose(1, 2)
        ...
        if return_ddm_attention:
            label_features, ddm_attention = self.dependency_module(
                label_features,
